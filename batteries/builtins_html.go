...
func do_html(es *env.ProgramState, reader io.Reader, dmap env.Dict) env.Object {
	...
	if ok {
		...
		case env.Block: // if values of start key is of type block, evaluate it
			...
			es.Ser = obj.Series
			evaldo.EvalBlockInj(es, *env.NewNative(es.Idx, "", "rye-html-start"), true)
			evaldo.MaybeDisplayFailureOrError(es, es.Idx, "do_html")
...
myloop:
				...
				case HtmlDialectNode: // if it is Dict
					...
					if (node.Condition == nil) || ((node.Condition.Class == "" || node.Condition.Class == class) &&
						...
						case TYPE_CODE: // if it is block
							...
							evaldo.EvalBlockInj(es, *env.NewNative(es.Idx, token, "rye-html-start"), true)
							evaldo.MaybeDisplayFailureOrError(es, es.Idx, "do_html\\code")
						...
						case TYPE_SUBNODE:
							...
							if ok {
								...
								case env.Block: // if values of start key is of type block, evaluate it
									...
									es.Ser = code.Series
									evaldo.EvalBlockInj(es, *env.NewNative(es.Idx, token, "rye-html-start"), true)
									evaldo.MaybeDisplayFailureOrError(es, es.Idx, "do_html\\subnode")
									if es.ErrorFlag {
										return es.Res
									}
									es.Ser = ser
									...
									case env.Dict: // if it is Dict
										...
										if ok {
											...
											case env.Block: // if values of start key is of type block, evaluate it
												ser := es.Ser // TODO -- make helper function that "does" a block
												es.Ser = obj.Series
												evaldo.EvalBlockInj(es, *env.NewNative(es.Idx, tok, "rye-html-start"), true)
												es.Ser = ser
											default:
												// TODO Err
											}
		...
		case html.TextToken:
			...
			if ok {
				...
				case HtmlDialectNode:
					...
					ser := es.Ser // TODO -- make helper function that "does" a block
					es.Ser = node.Code.Series
					evaldo.EvalBlockInj(es, *env.NewString(tok.Data), true)
					evaldo.MaybeDisplayFailureOrError(es, es.Idx, "do_html\\text-token")
					if es.ErrorFlag {
						return es.Res
					}
					es.Ser = ser
			...
			if inElement == curtag { // TODO -- solve the case of same named elements inside <person><person></person></person>
				...
				if ok {
					...
					case env.Block:
						...
						es.Ser = obj.Series
						evaldo.EvalBlockInj(es, *env.NewNative(es.Idx, tok, "rye-html-start"), true)
						evaldo.MaybeDisplayFailureOrError(es, es.Idx, "do_html\\end-tag-token")
	...
	if ok1 {
		...
		case HtmlDialectNode: // if values of start key is of type block, evaluate it
			...
			evaldo.EvalBlockInj(es, *env.NewNative(es.Idx, "", "rye-html-start"), true)
			evaldo.MaybeDisplayFailureOrError(es, es.Idx, "do_html\\return")
