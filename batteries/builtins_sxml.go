		...
		case env.Block:
			...
			if obj1.Mode == 1 {
				...
				if len(keys) > 0 {
					...
					return EmptyRM(), evaldo.MakeBuiltinError(ps, "No selectors before tag map.", "reader//do-sxml")
				}
			}
		default:
			// ni Dict ampak blok kode, vrni blok
			return EmptyRM(), evaldo.MakeBuiltinError(ps, "Unknown type in block parsing: "+obj.Inspect(*ps.Idx), "reader//do-sxml")
		}
	}
	return rmap, nil
}
		...
		case xml.StartElement:
			...
			if ok {
				...
				case env.Dict:
					...
					if ok {
						...
						case env.Block:
							...
							ps.Ser = obj.Series
							evaldo.EvalBlockInj(ps, *env.NewNative(ps.Idx, se, "rye-sxml-start"), true)
							evaldo.MaybeDisplayFailureOrError(ps, ps.Idx, "do-sxml 1")
							if ps.ErrorFlag {
								ps.Ser = ser
								return ps.Res
							}
				...
				case env.Block:
					...
					ps.Ser = obj.Series
					evaldo.EvalBlockInj(ps, *env.NewNative(ps.Idx, se, "rye-sxml-start"), true)
					evaldo.MaybeDisplayFailureOrError(ps, ps.Idx, "do-sxml 2")
					if ps.ErrorFlag {
						ps.Ser = ser
						return ps.Res
					}
				...
				if true || inElement == curtag {
					...
					if ok {
						...
						case env.Block:
							...
							ps.Ser = obj.Series
							evaldo.EvalBlockInj(ps, *env.NewNative(ps.Idx, se, "rye-sxml-start"), true)
							evaldo.MaybeDisplayFailureOrError(ps, ps.Idx, "do-sxml 3")
							if ps.ErrorFlag {
								ps.Ser = ser
								return ps.Res
							}
		...
		case xml.CharData:
			...
			if ok {
				...
				case env.Block:
					ser := ps.Ser // TODO -- make helper function that "does" a block
					ps.Ser = obj.Series
					evaldo.EvalBlockInj(ps, *env.NewString(string(se.Copy())), true)
					evaldo.MaybeDisplayFailureOrError(ps, ps.Idx, "do-sxml 5")
					if ps.ErrorFlag {
						ps.Ser = ser
						return ps.Res
					}
				...
				if len(handlerDepth) > 0 && handlerDepth[len(handlerDepth)-1] == depth {
					...
					if ok {
						...
						case env.Block:
							...
							ps.Ser = obj.Series
							evaldo.EvalBlockInj(ps, *env.NewNative(ps.Idx, se, "rye-sxml-start"), true)
							evaldo.MaybeDisplayFailureOrError(ps, ps.Idx, "do-sxml 6")
							if ps.ErrorFlag {
								ps.Ser = ser
								return ps.Res
							}
