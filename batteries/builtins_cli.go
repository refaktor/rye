...
func validateWithCheck(es *env.ProgramState, value env.Object, spec *ArgSpec) error {
	...
	evaldo.EvalBlockInj(es, value, true)
...
func BuiParseArgs(es *env.ProgramState, args env.Object, specBlock env.Object) env.Object {
	...
	if !ok {
		es.FailureFlag = true
		return evaldo.MakeArgError(es, 1, []env.Type{env.BlockType}, "parse-args")
	}

	spec, ok := specBlock.(env.Block)
	if !ok {
		es.FailureFlag = true
		return evaldo.MakeArgError(es, 2, []env.Type{env.BlockType}, "parse-args")
	}
