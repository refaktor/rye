...
func conversion_evalWord(word env.Word, ps *env.ProgramState, vals env.Object) (env.Object, any) {
	...
	case "calc":
		...
		case env.Block:
			ser := ps.Ser
			ps.Ser = blk.Series
			evaldo.EvalBlockInj(ps, vals, true)
			evaldo.MaybeDisplayFailureOrError(ps, ps.Idx, "conversion eval word")
			if ps.ErrorFlag {
				ps.Ser = ser
				return ps.Res, nil
			}
...
func BuiConvert(ps *env.ProgramState, arg0 env.Object, arg1 env.Object) env.Object {
	...
	case env.Block:
		...
		default:
			return evaldo.MakeArgError(ps, 1, []env.Type{env.DictType, env.ContextType}, "BuiConvert")
		}
		ps.Ser = ser1
		return vals
	default:
		return evaldo.MakeArgError(ps, 2, []env.Type{env.BlockType}, "BuiConvert")
	}
}
