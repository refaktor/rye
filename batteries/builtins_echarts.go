...
func blockToLineData(ps *env.ProgramState, blk env.Block) ([]opts.LineData, *env.Error) {
	...
	for i := 0; i < blk.Series.Len(); i++ {
		...
		case env.Decimal:
			items = append(items, opts.LineData{Value: v.Value})
		default:
			return nil, evaldo.MakeBuiltinError(ps, "line-data block must contain integers or decimals", "line-data")
		}
	}
	return items, nil
}
...
func blockToBarData(ps *env.ProgramState, blk env.Block) ([]opts.BarData, *env.Error) {
	...
	for i := 0; i < blk.Series.Len(); i++ {
		...
		case env.Decimal:
			items = append(items, opts.BarData{Value: v.Value})
		default:
			return nil, evaldo.MakeBuiltinError(ps, "bar-data block must contain integers or decimals", "bar-data")
		}
	}
	return items, nil
}
...
func blockToPieData(ps *env.ProgramState, blk env.Block) ([]opts.PieData, *env.Error) {
	...
	for i := 0; i < blk.Series.Len(); i += 2 {
		if i+1 >= blk.Series.Len() {
			return nil, evaldo.MakeBuiltinError(ps, "pie-data needs pairs of name and value", "pie-data")
		}
		...
		if !ok {
			return nil, evaldo.MakeBuiltinError(ps, "pie-data name must be string", "pie-data")
		}
		...
		case env.Decimal:
			value = v.Value
		default:
			return nil, evaldo.MakeBuiltinError(ps, "pie-data value must be integer or decimal", "pie-data")
		}
...
func blockToScatterData(ps *env.ProgramState, blk env.Block) ([]opts.ScatterData, *env.Error) {
	...
	for i := 0; i < blk.Series.Len(); i++ {
		...
		if !ok || pointBlk.Series.Len() != 2 {
			return nil, evaldo.MakeBuiltinError(ps, "scatter-data needs blocks of 2 values [x y]", "scatter-data")
		}
		...
		case env.Decimal:
			x = v.Value
		default:
			return nil, evaldo.MakeBuiltinError(ps, "scatter x value must be integer or decimal", "scatter-data")
		}
		...
		case env.Decimal:
			y = v.Value
		default:
			return nil, evaldo.MakeBuiltinError(ps, "scatter y value must be integer or decimal", "scatter-data")
		}
...
func blockToStringSlice(ps *env.ProgramState, blk env.Block) ([]string, *env.Error) {
	...
	for i := 0; i < blk.Series.Len(); i++ {
		...
		default:
			return nil, evaldo.MakeBuiltinError(ps, "block must contain strings", "set-x-axis")
		}
	}
	return items, nil
}
