...
func matchValue(ps *env.ProgramState, value env.Object, pattern env.Object, mmode matchMode) (env.Object, matchMode) {
	...
	if patternIsBlock {
		if !valueIsBlock {
			ps.FailureFlag = true
			return evaldo.MakeBuiltinError(ps, "Pattern is a block but value is not", "match"), mmode
		}

		// Both are blocks - check if they have the same length
		// if valueBlock.Series.Len() != patternBlock.Series.Len() {
		//	ps.FailureFlag = true
		//	return evaldo.MakeBuiltinError(ps, "Block and pattern have different lengths", "match"), mmode
		// }
		...
		if j == valueBlock.Series.Len() || mmode.tailMode == 1 {
			...
			return evaldo.MakeError(ps, "Pattern too short for data"), mmode
		}
	}
	...
	case env.Getword:
		...
		if !found {
			ps.FailureFlag = true
			return evaldo.MakeBuiltinError(ps, "Word not found in context: "+ps.Idx.GetWord(word.Index), "match"), mmode
		}
		if !contextValue.Equal(value) {
			ps.FailureFlag = true
			return evaldo.MakeBuiltinError(ps, "Value doesn't match context value", "match"), mmode
		}
	case env.LModword:
		// Get the value from the context and check if it equals the value
	...
	case env.Opword:
		...
		if int(value.Type()) != word.Index {
			ps.FailureFlag = true
			return evaldo.MakeBuiltinError(ps, "Expected type "+ps.Idx.GetWord(word.Index), "match"), mmode
		}
	case env.Block:
	...
	default:
		...
		if !pattern.Equal(value) {
			ps.FailureFlag = true
			return evaldo.MakeBuiltinError(ps, "Pattern doesn't match value", "match"), mmode
		}
	}
