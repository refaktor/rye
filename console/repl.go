...
func (r *Repl) evalLine(es *env.ProgramState, code string) string {
	...
	if es.LiveObj != nil {
		...
		for _, update := range es.LiveObj.Updates {
			...
			block_, script_ := evaldo.LoadScriptLocalFile(es, *env.NewUri1(es.Idx, "file://"+update))
			es.Res = evaldo.EvaluateLoadedValue(es, block_, script_, true)
		}
	...
	if multiline {
		...
		if r.dialect == "rye" {
			...
			evaldo.EvalBlockInj(es, r.prevResult, true)
			es.InErrHandler = false
		} else if r.dialect == "eyr" {
			es.Dialect = env.EyrDialect
			evaldo.BatteryEyrEvalBlockHook(es, true)
			...
			res := evaldo.BatteryDialectMathHook(es, block1)
			...
			case env.Block:
				...
				ser := es.Ser
				es.Ser = block.Series
				evaldo.BatteryEyrEvalBlockHook(es, false)
				es.Ser = ser
			}
		} else {
...
// DoRyeDualRepl is a placeholder for the dual-pane REPL (full implementation in repl_dual.go.TODO).
// Falls back to a single REPL on the left ProgramState.
func DoRyeDualRepl(leftPs *env.ProgramState, rightPs *env.ProgramState, dialect string, showResults bool) {
	DoRyeRepl(leftPs, dialect, showResults, false, "")
}
