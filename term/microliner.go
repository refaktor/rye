...
// historySearch implements a two-stage Ctrl+R behavior:
// 1. Search mode: user types pattern, see incremental results
// 2. Browse mode: navigate through filtered results
func (s *MLState) historySearch(p []rune, line []rune, pos int) ([]rune, int, KeyEvent, error) {
	const numLines = 4
	...
	// Hide cursor during search mode
	s.sendBack("\033[?25l") // Hide cursor
	...
	// renderSearchDisplay shows search prompt and current matches
	renderSearchDisplay := func() {
		matches, positions := s.getHistoryByPattern(searchPattern)
		...
		// Top line shows search prompt
		prompt := fmt.Sprintf("🔍 search: \033[1m%s\033[0m\033[36m [tab] to browse %s", searchPattern, what)
		if len(prompt) > maxW {
			prompt = "🔍 ***" + searchPattern[len(searchPattern)-(maxW-4):]
		}
		content[numLines-1] = "\033[36m" + prompt + "\033[0m"

		// Show up to 3 matches below the prompt
		for i := 0; i < 3 && i < len(matches); i++ {
			entry := matches[i]
			runes := []rune(entry)

			// Highlight the search pattern in the result
			if searchPattern != "" && i < len(positions) {
				pos := positions[i]
				before := string(runes[:pos])
				match := searchPattern
				after := ""
				if pos+len(searchPattern) < len(runes) {
					after = string(runes[pos+len(searchPattern):])
				}

				// Truncate if too long
				fullEntry := before + match + after
				if len([]rune(fullEntry)) > maxW {
					// Simple truncation - could be improved
					fullEntry = string([]rune(fullEntry)[:maxW-3]) + "..."
				}

				// Rebuild with highlighting
				if len([]rune(fullEntry)) <= maxW {
					highlighted := before + "\033[43;30m" + match + "\033[0m\033[90m" + after
					entry = highlighted
				}
			} else if len(runes) > maxW {
				entry = string(runes[:maxW-3]) + "..."
			}

			// Bottom is most recent (i=0), top is oldest visible (i=2)
			lineIdx := 2 - i // convert to content array index (2,1,0)
			if i == 0 {
				content[lineIdx] = "\033[38;5;37m▶ " + entry + "\033[0m"
			} else {
				content[lineIdx] = "\033[90m  " + entry + "\033[0m"
			}
		}
	...
	for {
		...
		switch {
		case next.Code == 27: // Escape - cancel
			s.sendBack("\033[?25h") // Show cursor
			s.refresh(p, line, pos) //nolint:errcheck
			return line, pos, KeyEvent{Code: 27}, nil

		case next.Code == 9: // Tab - also go to browse mode
			matches, _ := s.getHistoryByPattern(searchPattern)
			if len(matches) > 0 {
				s.sendBack("\033[?25h") // Show cursor
				return s.historyBrowseFiltered(p, line, pos, matches)
			} else {
				s.sendBack("\033[?25h") // Show cursor
				return s.historyBrowse(p, line, pos)
			}
			// No matches, cancel
			s.sendBack("\033[?25h") // Show cursor
			s.refresh(p, line, pos) //nolint:errcheck
		...
		case next.Ctrl && strings.ToLower(next.Key) == "r": // Ctrl+R - accept top result
			matches, _ := s.getHistoryByPattern(searchPattern)
			if len(matches) > 0 {
				selectedLine := []rune(matches[0])
				s.sendBack("\033[?25h") // Show cursor
				return selectedLine, len(selectedLine), KeyEvent{Code: 27}, nil
			}
			// No matches, cancel
			s.sendBack("\033[?25h") // Show cursor
			s.refresh(p, line, pos) //nolint:errcheck
		...
		case next.Code == 8: // Backspace
			if len(searchPattern) > 0 {
				searchPattern = searchPattern[:len(searchPattern)-1]
				renderSearchDisplay()
			}

		case len(next.Key) == 1 && !next.Ctrl && !next.Alt: // Regular character
			searchPattern += next.Key
			renderSearchDisplay()

		default:
			// Any other key: if we have matches, accept top result and pass key through
			matches, _ := s.getHistoryByPattern(searchPattern)
			if len(matches) > 0 {
				selectedLine := []rune(matches[0])
				s.sendBack("\033[?25h") // Show cursor
				return selectedLine, len(selectedLine), next, nil
			}
			// No matches, cancel and pass key through
			s.sendBack("\033[?25h") // Show cursor
	...
	for {
		...
		haveNext:
			if next.Ctrl {
				...
				case "n":
					histNext()
				case "r": // Ctrl+R → history search with browse
					line, pos, next, _ = s.historySearch(p, line, pos)
					goto haveNext
